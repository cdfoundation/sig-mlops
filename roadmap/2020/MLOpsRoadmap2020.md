...
- Tooling should enable appropriate governance processes around the release of ML assets into team environments.</br></td>
  ...
  <tr>
    ...
    <td>Training sets are prepared data sets which are extracted from production data. They may contain PII or sensitive information, so making the data available to developers in a way analagous to source code may be problematic. However, we can use either links to immutable blob stores, volumes or content addressible schemes (such as a hash of content) in versioned assets which can be stored as text in a Git Repository to provide version control and history for these assets. Training scripts are naturally source code (even if a visual tool is used, its likely format on disk is textual or some markup language). All scripts should be versioned in the same repository as training set data (or links to immutable sets of data), as the scripts to prepare features from the data are intimately connected to it. Script should also include information such as test and validation splits (they often are random sampling, but for reproducibility it would be ideal to have the models reproducible from the training set + scripts). Service wrappers typically are "ordinary" source code, but should be versioned along side scripts and training sets as often the data handling to serve predictions and analysis are related. Usage of Git for all concerns like this is sometimes known in the industry as "GitOps".</td>
  </tr>
  <tr>
